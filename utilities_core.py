...
class llm_setup():
    def __init__(self, server : str = "openai", messages : list = None):
        self.server         = server
    ...
    def initiate(self):
        if self.server == "openai":
            llm = self.openai_client.chat.completions.create(model          = "gpt-4o",
                                                            temperature     = 0,
                                                            max_tokens      = 4000,
...
if __name__=="__main__":
    # _text, metadata = doc_parse(".\data\ConceptsofBiology-WEB.pdf")
    context = """Sourav Chandidas Ganguly[1][2] (/sʃuːrəv ɡɛnɡuːlj/ ⓘ; natively spelled as Gangopadhyay; born 8 July 1972), also known as Dada (meaning "elder brother" in Bengali), is an Indian cricket commentator and former cricketer. He is popularly called the Maharaja of Indian Cricket.[3] He was captain of the Indian national cricket team and is regarded as one of India's most successful cricket captains.[4] As captain, he led Indian national team to win the 2002 ICC Champions Trophy and reach the final of the 2003 Cricket World Cup, the 2000 ICC Champions Trophy and the 2004 Asia Cup.[5]"""
    ...
    msg= [
        ...
        {
            "role": "user",
            "content": """Who is Sourabh ganguly?"""
        }
    ]
